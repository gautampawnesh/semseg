...
class CustomConcatDataset(ConcatDataset):
    ...
    def __init__(self, **kwargs):
        datasets = []
        for dataset_cfg in kwargs["datasets"]:
            dataset_type = dataset_cfg["type"]
            dataset_obj_cls = DATASETS.get(dataset_type)
            dataset_cfg.pop("type")
            datasets.append(dataset_obj_cls(**dataset_cfg))
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, indice in enumerate(indices):
            try:
                ...
                er.args += (preds, "indices", indices, "datasetidx", dataset_idx, sample_idx)
